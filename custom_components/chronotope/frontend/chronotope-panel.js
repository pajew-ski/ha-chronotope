...
.marker-cluster span {
    ...
    `]);customElements.define("chronotope-map-view",Pe);var Fs=["Mo","Di","Mi","Do","Fr","Sa","So"],ke=class extends Q{constructor(){super(),this.profiles=[],this.selectedProfileId="",this._profileName=""}updated(r){if(r.has("selectedProfileId")){let l=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this._profileName=l?l.name:""}}render(){let r=this.state;return $`
      <div class="groups">
        <div class="group">
    ...
    `}_saveProfile(){let r=(this._profileName||"").trim();if(!r)return;let l=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this.dispatchEvent(new CustomEvent("profile-save",{detail:{name:r,id:l&&l.name===r?l.id:void 0}}))}_toggleCategory(r){let l=this.state.categories.includes(r)?this.state.categories.filter(o=>o!==r):[...this.state.categories,r];this._patch({categories:l})}_toggleWeekday(r){let l=this.state.weekdays.includes(r)?this.state.weekdays.filter(o=>o!==r):[...this.state.weekdays,r];this._patch({weekdays:l})}};et(ke,"properties",{state:{attribute:!1},categories:{attribute:!1},icsCopied:{attribute:!1},profiles:{attribute:!1},selectedProfileId:{attribute:!1},_profileName:{state:!0}}),et(ke,"styles",st`
    :host {
      display: block;
      background: var(--card-background-color, #fff);
  ...
  `);customElements.define("chronotope-filter-bar",ke);var Ce=class extends Q{render(){let r=this.events||[];return r.length===0?$`<div class="empty">Keine Events für die aktuellen Filter.</div>`:$`${r.map(l=>this._renderItem(l))}`}_renderItem(r){let l=r.occurrences?.[0]?.[0]??r.start_time,o=r.occurrences?.[0]?.[1]??r.end_time;return $`
      <button
        class="item"
  ...
  `);customElements.define("chronotope-event-editor",Se);var Zo=(d,r)=>d.callWS({type:"chronotope/events/query",...r}),Bo=d=>d.callWS({type:"chronotope/categories"}),No=(d,r)=>d.callWS({type:"chronotope/ics_url",...r}),Do=(d,r)=>d.callWS({type:"chronotope/events/save",event:r}),Ro=(d,r)=>d.callWS({type:"chronotope/events/delete",event_id:r}),Fo=(d,r,l)=>d.callWS({type:"chronotope/events/flag",event_id:r,...l}),Ho=d=>d.callWS({type:"chronotope/profiles/list"}),Uo=(d,r)=>d.callWS({type:"chronotope/profiles/save",profile:r}),Go=(d,r)=>d.callWS({type:"chronotope/profiles/delete",profile_id:r});function Qe(d){let r={};return d.categories.length&&(r.categories=[...d.categories]),d.radiusEnabled&&d.center&&(r.center={lat:d.center.lat,lon:d.center.lon},r.radius_km=d.radiusKm),d.start&&(r.start=new Date(d.start).toISOString()),d.end&&(r.end=new Date(d.end).toISOString()),d.weekdays.length&&(r.weekdays=[...d.weekdays]),d.timeMode==="range"&&(d.timeFrom&&(r.time_from=d.timeFrom),d.timeTo&&(r.time_to=d.timeTo)),d.text&&(r.text=d.text),d.favoritesOnly&&(r.favorites_only=!0),r}var Hs=250,Us=new Set(["showZones","dayFilter"]);function Wo(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let l=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${l(r.getMonth()+1)}-${l(r.getDate())}T${l(r.getHours())}:${l(r.getMinutes())}`}var Te=class extends Q{constructor(){super(),this._events=[],this._categories=[],this._selectedId=null,this._icsCopied=!1,this._error=null,this._profiles=[],this._selectedProfileId="",this._editing=null,this._capture=null,this._filters={categories:[],radiusEnabled:!1,radiusKm:10,center:null,start:"",end:"",weekdays:[],timeMode:"allday",timeFrom:"",timeTo:"",text:"",favoritesOnly:!1,showZones:!0,dayFilter:""},this._initialized=!1}willUpdate(r){r.has("hass")&&this.hass&&!this._initialized&&(this._initialized=!0,this._filters={...this._filters,center:{lat:this.hass.config.latitude,lon:this.hass.config.longitude}},this._loadCategories(),this._loadProfiles(),this._runQuery())}get _displayedEvents(){if(!this._filters.dayFilter)return this._events;let r=new Date(`${this._filters.dayFilter}T00:00:00`),l=new Date(r.getTime()+864e5);return this._events.filter(o=>(o.occurrences||[[o.start_time,o.end_time]]).some(([u,c])=>new Date(u)<l&&new Date(c)>r))}render(){let r=!!this.hass?.themes?.darkMode,l=this._displayedEvents;return $`
      <header>
        <h1>Chronotope</h1>
        <span class="count">
