...
.marker-cluster span {
    ...
    `]);customElements.define("chronotope-map-view",ke);var Ce=class extends tt{constructor(){super(),this.profiles=[],this.selectedProfileId="",this._profileName=""}updated(r){if(r.has("selectedProfileId")){let a=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this._profileName=a?a.name:""}}render(){let r=this.state;return Z`
      <div class="groups">
        <div class="group">
    ...
    `}_saveProfile(){let r=(this._profileName||"").trim();if(!r)return;let a=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this.dispatchEvent(new CustomEvent("profile-save",{detail:{name:r,id:a&&a.name===r?a.id:void 0}}))}_toggleCategory(r){let a=this.state.categories.includes(r)?this.state.categories.filter(o=>o!==r):[...this.state.categories,r];this._patch({categories:a})}_toggleWeekday(r){let a=this.state.weekdays.includes(r)?this.state.weekdays.filter(o=>o!==r):[...this.state.weekdays,r];this._patch({weekdays:a})}};it(Ce,"properties",{state:{attribute:!1},categories:{attribute:!1},icsCopied:{attribute:!1},profiles:{attribute:!1},selectedProfileId:{attribute:!1},stats:{attribute:!1},_profileName:{state:!0}}),it(Ce,"styles",at`
    :host {
      display: block;
      background: var(--card-background-color, #fff);
  ...
  `);customElements.define("chronotope-filter-bar",Ce);var Se=class extends tt{render(){let r=this.events||[];return r.length===0?Z`<div class="empty">${w("list.empty")}</div>`:Z`${r.map(a=>this._renderItem(a))}`}_renderItem(r){let a=r.occurrences?.[0]?.[0]??r.start_time,o=r.occurrences?.[0]?.[1]??r.end_time;return Z`
      <button
        class="item"
      ...
      >
        ...
        <div class="meta">
          ...
          ${r.recurrence?Z`<span title=${r.recurrence}>🔁</span>`:I}
  ...
  `);customElements.define("chronotope-event-editor",Te);var Ro=(d,r)=>d.callWS({type:"chronotope/events/query",...r}),Fo=d=>d.callWS({type:"chronotope/categories"}),Ho=(d,r)=>d.callWS({type:"chronotope/ics_url",...r}),Uo=(d,r)=>d.callWS({type:"chronotope/events/save",event:r}),Go=(d,r)=>d.callWS({type:"chronotope/events/delete",event_id:r}),Wo=(d,r,a)=>d.callWS({type:"chronotope/events/flag",event_id:r,...a}),qo=d=>d.callWS({type:"chronotope/profiles/list"}),jo=(d,r)=>d.callWS({type:"chronotope/profiles/save",profile:r}),Vo=(d,r)=>d.callWS({type:"chronotope/profiles/delete",profile_id:r}),Ko=d=>d.callWS({type:"chronotope/stats"});function ei(d){let r={};return d.categories.length&&(r.categories=[...d.categories]),d.radiusEnabled&&d.center&&(r.center={lat:d.center.lat,lon:d.center.lon},r.radius_km=d.radiusKm),d.start&&(r.start=new Date(d.start).toISOString()),d.end&&(r.end=new Date(d.end).toISOString()),d.weekdays.length&&(r.weekdays=[...d.weekdays]),d.timeMode==="range"&&(d.timeFrom&&(r.time_from=d.timeFrom),d.timeTo&&(r.time_to=d.timeTo)),d.text&&(r.text=d.text),d.favoritesOnly&&(r.favorites_only=!0),r}var qs=250,js=new Set(["showZones","showPersons","showGeoFeeds","dayFilter"]);function Yo(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Me=class extends tt{constructor(){super(),this._events=[],this._categories=[],this._selectedId=null,this._icsCopied=!1,this._error=null,this._profiles=[],this._selectedProfileId="",this._editing=null,this._capture=null,this._stats=null,this._filters={categories:[],radiusEnabled:!1,radiusKm:10,center:null,start:"",end:"",weekdays:[],timeMode:"allday",timeFrom:"",timeTo:"",text:"",favoritesOnly:!1,showZones:!0,showPersons:!0,showGeoFeeds:!0,dayFilter:""},this._initialized=!1}willUpdate(r){r.has("hass")&&this.hass&&Zo(this.hass.locale?.language||this.hass.language),r.has("hass")&&this.hass&&!this._initialized&&(this._initialized=!0,this._filters={...this._filters,center:{lat:this.hass.config.latitude,lon:this.hass.config.longitude}},this._loadCategories(),this._loadProfiles(),this._loadStats(),this._runQuery())}async _loadStats(){try{this._stats=await Ko(this.hass)}catch(r){console.error("chronotope: loading stats failed",r)}}get _displayedEvents(){if(!this._filters.dayFilter)return this._events;let r=new Date(`${this._filters.dayFilter}T00:00:00`),a=new Date(r.getTime()+864e5);return this._events.filter(o=>(o.occurrences||[[o.start_time,o.end_time]]).some(([u,c])=>new Date(u)<a&&new Date(c)>r))}render(){let r=!!this.hass?.themes?.darkMode,a=this._displayedEvents;return Z`
      <header>
        <h1>Chronotope</h1>
        <span class="count">
    ...
    `}_haZones(){let r=this.hass?.states||{},a=Object.values(r).filter(o=>o.entity_id.startsWith("zone.")).map(o=>({id:o.entity_id,name:o.attributes.friendly_name||o.entity_id,lat:o.attributes.latitude,lon:o.attributes.longitude,radius:o.attributes.radius??100,passive:!!o.attributes.passive,home:o.entity_id==="zone.home"})).filter(o=>o.lat!=null&&o.lon!=null);return!a.some(o=>o.home)&&this.hass?.config?.latitude!=null&&a.push({id:"home",name:w("panel.home"),lat:this.hass.config.latitude,lon:this.hass.config.longitude,radius:100,passive:!1,home:!0}),a}_haPersons(){let r=this.hass?.states||{};return Object.values(r).filter(a=>a.entity_id.startsWith("person.")).map(a=>({id:a.entity_id,name:a.attributes.friendly_name||a.entity_id,lat:a.attributes.latitude,lon:a.attributes.longitude,picture:a.attributes.entity_picture||null,state:a.state})).filter(a=>a.lat!=null&&a.lon!=null)}_haGeoLocations(){let r=this.hass?.states||{};return Object.values(r).filter(a=>a.entity_id.startsWith("geo_location.")).map(a=>({id:a.entity_id,name:a.attributes.friendly_name||a.entity_id,lat:a.attributes.latitude,lon:a.attributes.longitude,source:a.attributes.source||"geo_location",distance:a.state,unit:a.attributes.unit_of_measurement||"km"})).filter(a=>a.lat!=null&&a.lon!=null)}_onFiltersChanged(r){this._filters={...this._filters,...r.detail},this._icsCopied=!1,Object.keys(r.detail).filter(o=>!js.has(o)).length&&this._scheduleQuery()}_onMapClick(r){let{lat:a,lon:o}=r.detail;if(this._capture?.mode==="point"){this._editorElement()?.setCoords(a,o),this._capture=null;return}if(this._capture?.mode){this._capture={...this._capture,points:[...this._capture.points,[a,o]]};return}this._filters={...this._filters,center:{lat:a,lon:o}},this._filters.radiusEnabled&&this._scheduleQuery()}_onEventSelected(r){this._selectedId=r.detail.id}_scheduleQuery(){clearTimeout(this._queryTimer),this._queryTimer=setTimeout(()=>this._runQuery(),qs)}async _loadCategories(){try{let r=await Fo(this.hass);this._categories=r.categories}catch(r){console.error("chronotope: loading categories failed",r)}}async _loadProfiles(){try{let r=await qo(this.hass);this._profiles=r.profiles}catch(r){console.error("chronotope: loading profiles failed",r)}}async _runQuery(){if(this.hass)try{let r=await Ro(this.hass,ei(this._filters));this._events=r.events,this._error=null,this._selectedId&&!this._events.some(a=>a.id===this._selectedId)&&(this._selectedId=null)}catch(r){this._error=w("error.query",{msg:r.message||r.code||r})}}_onProfileSelected(r){this._selectedProfileId=r.detail.id;let a=this._profiles.find(o=>o.id===r.detail.id);a&&this._applyProfileFilters(a.filters||{})}_applyProfileFilters(r){let a=r.center||this._filters.center;this._filters={...this._filters,categories:r.categories||[],radiusEnabled:!!(r.center&&r.radius_km!=null),radiusKm:r.radius_km!=null?r.radius_km:this._filters.radiusKm,center:a,start:Yo(r.start),end:Yo(r.end),weekdays:r.weekdays||[],timeMode:r.time_from||r.time_to?"range":"allday",timeFrom:r.time_from||"",timeTo:r.time_to||"",text:r.text||"",favoritesOnly:!!r.favorites_only,dayFilter:""},this._scheduleQuery()}async _onProfileSave(r){try{let a=await jo(this.hass,{id:r.detail.id,name:r.detail.name,filters:ei(this._filters)});await this._loadProfiles(),this._selectedProfileId=a.profile.id,this._error=null}catch(a){this._error=w("error.profileSave",{msg:a.message||a.code||a})}}async _onProfileDelete(r){try{await Vo(this.hass,r.detail.id),this._selectedProfileId===r.detail.id&&(this._selectedProfileId=""),await this._loadProfiles()}catch(a){this._error=w("error.profileDelete",{msg:a.message||a.code||a})}}_editorElement(){return this.renderRoot.querySelector("chronotope-event-editor")}_onNewEvent(){let r=new Date;r.setMinutes(0,0,0);let a=new Date(r.getTime()+2*36e5);this._editing={start_time:r.toISOString(),end_time:a.toISOString()},this._capture=null}_onEventEdit(r){let a=this._events.find(o=>o.id===r.detail.id);a&&(this._editing=a,this._capture=null)}async _onEditorSave(r){try{await Uo(this.hass,r.detail.event),this._editing=null,this._capture=null,this._error=null,await this._runQuery(),await this._loadCategories()}catch(a){this._error=w("error.save",{msg:a.message||a.code||a})}}async _onEditorDelete(r){try{await Go(this.hass,r.detail.id),this._editing=null,this._capture=null,await this._runQuery()}catch(a){this._error=w("error.delete",{msg:a.message||a.code||a})}}_onEditorCancel(){this._editing=null,this._capture=null}_onCaptureRequest(r){let a=r.detail.mode;this._capture=a?{mode:a,points:[]}:null}_onCaptureFinish(){let r=this._capture;if(r){if(r.mode==="line"&&r.points.length>=2)this._editorElement()?.setGeometry({type:"LineString",coordinates:r.points.map(([a,o])=>[o,a])});else if(r.mode==="polygon"&&r.points.length>=3){let a=r.points.map(([o,l])=>[l,o]);a.push(a[0]),this._editorElement()?.setGeometry({type:"Polygon",coordinates:[a]})}else{this._error=w("error.capture");return}this._capture=null,this._error=null}}async _onEventFlag(r){let{id:a,...o}=r.detail;try{await Wo(this.hass,a,o),await this._runQuery()}catch(l){this._error=w("error.action",{msg:l.message||l.code||l})}}async _onIcsRequested(){try{let r=this._selectedProfileId?{profile_id:this._selectedProfileId}:ei(this._filters),a=await Ho(this.hass,r);await navigator.clipboard.writeText(a.url),this._icsCopied=!0,setTimeout(()=>{this._icsCopied=!1},3e3)}catch(r){this._error=w("error.ics",{msg:r.message||r.code||r})}}};it(Me,"properties",{hass:{attribute:!1},narrow:{attribute:!1},route:{attribute:!1},panel:{attribute:!1},_events:{state:!0},_categories:{state:!0},_filters:{state:!0},_selectedId:{state:!0},_icsCopied:{state:!0},_error:{state:!0},_profiles:{state:!0},_selectedProfileId:{state:!0},_editing:{state:!0},_capture:{state:!0},_stats:{state:!0}}),it(Me,"styles",at`
    :host {
      display: flex;
      flex-direction: column;
      height: 100vh;
  ...
  `);customElements.define("chronotope-panel",Me);
