...
.marker-cluster span {
	line-height: 30px;
	}`;var Pe=class extends Q{constructor(){super(),this.events=[],this.radiusKm=10,this.radiusEnabled=!1,this.zones=[],this.persons=[],this.geoMarkers=[],this.capture=null,this.dark=!1,this._markersById=new Map,this._didInitialFit=!1,this._zonesSignature="",this._personsSignature="",this._geoSignature=""}render(){return I`<div id="map"></div>`}firstUpdated(){let r=this.center?[this.center.lat,this.center.lon]:[52.52,13.405];this._map=H.default.map(this.renderRoot.getElementById("map"),{center:r,zoom:12,zoomControl:!0}),H.default.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'}).addTo(this._map),this._zoneLayer=H.default.layerGroup().addTo(this._map),this._geoFeedLayer=H.default.layerGroup().addTo(this._map),this._personLayer=H.default.layerGroup().addTo(this._map),this._shapeLayer=H.default.featureGroup().addTo(this._map),this._clusterGroup=H.default.markerClusterGroup({maxClusterRadius:40,showCoverageOnHover:!1}).addTo(this._map),this._radiusLayer=H.default.layerGroup().addTo(this._map),this._captureLayer=H.default.layerGroup().addTo(this._map),this._map.on("click",a=>{this.dispatchEvent(new CustomEvent("map-click",{detail:{lat:a.latlng.lat,lon:a.latlng.lng}}))}),this._resizeObserver=new ResizeObserver(()=>this._map.invalidateSize()),this._resizeObserver.observe(this),this._renderEvents(),this._renderRadius(),this._renderZones(),this._renderPersons(),this._renderGeoMarkers()}disconnectedCallback(){super.disconnectedCallback(),this._resizeObserver?.disconnect(),this._map?.remove(),this._map=void 0}updated(r){this._map&&(r.has("events")&&this._renderEvents(),(r.has("center")||r.has("radiusKm")||r.has("radiusEnabled"))&&this._renderRadius(),r.has("zones")&&this._renderZones(),r.has("persons")&&this._renderPersons(),r.has("geoMarkers")&&this._renderGeoMarkers(),r.has("capture")&&this._renderCapture(),r.has("selectedId")&&this.selectedId&&this._focusEvent(this.selectedId))}_accentColor(){return getComputedStyle(this).getPropertyValue("--primary-color").trim()||"#03a9f4"}_renderEvents(){this._clusterGroup.clearLayers(),this._shapeLayer.clearLayers(),this._markersById.clear();let r=this._accentColor();for(let a of this.events||[]){let o=null,l=!1;if(a.geometry)try{o=H.default.geoJSON(JSON.parse(a.geometry),{style:this._shapeStyle(r,a),pointToLayer:(u,c)=>H.default.circleMarker(c,this._markerStyle(r,a))})}catch(u){console.warn("chronotope: invalid geometry for event",a.id,u)}!o&&a.lat!=null&&a.lon!=null&&(o=H.default.circleMarker([a.lat,a.lon],this._markerStyle(r,a)),l=!0),o&&(o.bindPopup(this._popupHtml(a)),o.on("click",()=>{this.dispatchEvent(new CustomEvent("event-selected",{detail:{id:a.id}}))}),(l?this._clusterGroup:this._shapeLayer).addLayer(o),this._markersById.set(a.id,{layer:o,clustered:l}))}if(!this._didInitialFit&&this._markersById.size>0){this._didInitialFit=!0;let a=this._clusterGroup.getBounds().extend(this._shapeLayer.getBounds());a.isValid()&&this._map.fitBounds(a.pad(.2),{maxZoom:14})}}_markerStyle(r,a){return{radius:9,color:r,weight:2,fillColor:r,fillOpacity:a?.favorite?.75:.35,dashArray:a?.time_precision==="approximate"?"3 4":null}}_shapeStyle(r,a){return{color:r,weight:3,fillOpacity:.2,dashArray:a?.time_precision==="approximate"?"6 6":null}}_popupHtml(r){let a=document.createElement("div"),o=document.createElement("div");o.className="popup-title",o.textContent=`${r.favorite?"\u2605 ":""}${r.title}`;let l=document.createElement("div");l.className="popup-meta";let u=r.occurrences?.[0]?.[0]??r.start_time,c=r.time_precision==="approximate"&&r.schedule_text?`~ ${r.schedule_text}`:new Date(u).toLocaleString();if(l.textContent=`${r.category||""} ${c}`.trim(),a.append(o,l),r.address){let _=document.createElement("div");_.className="popup-meta",_.textContent=r.address,a.append(_)}if(r.lat!=null&&r.lon!=null){let _=document.createElement("div");_.className="popup-meta";let g=document.createElement("a");g.href=`https://www.openstreetmap.org/directions?to=${r.lat}%2C${r.lon}`,g.target="_blank",g.rel="noopener noreferrer",g.textContent="\u{1F9ED} Route (OSM)",_.append(g),a.append(_)}return a}_renderRadius(){if(this._radiusLayer.clearLayers(),!this.radiusEnabled||!this.center)return;let r=this._accentColor();H.default.circle([this.center.lat,this.center.lon],{radius:this.radiusKm*1e3,color:r,weight:1.5,dashArray:"6 6",fillOpacity:.05}).addTo(this._radiusLayer),H.default.circleMarker([this.center.lat,this.center.lon],{radius:4,color:r,fillColor:r,fillOpacity:1}).addTo(this._radiusLayer)}_renderZones(){if(!this._map)return;let r=JSON.stringify(this.zones||[]);if(r===this._zonesSignature)return;this._zonesSignature=r,this._zoneLayer.clearLayers();let a=getComputedStyle(this).getPropertyValue("--accent-color").trim()||"#ff9800";for(let o of this.zones||[])H.default.circle([o.lat,o.lon],{radius:o.radius,color:a,weight:1.5,dashArray:o.passive?"2 6":"4 4",fillColor:a,fillOpacity:.06}).bindTooltip(o.name).addTo(this._zoneLayer),o.home&&H.default.marker([o.lat,o.lon],{icon:H.default.divIcon({className:"zone-home-icon",html:"\u{1F3E0}",iconSize:[24,24],iconAnchor:[12,12]}),interactive:!1,keyboard:!1}).addTo(this._zoneLayer)}_renderPersons(){if(!this._map)return;let r=JSON.stringify(this.persons||[]);if(r!==this._personsSignature){this._personsSignature=r,this._personLayer.clearLayers();for(let a of this.persons||[]){let o=a.picture?`<img src="${a.picture}" alt="" />`:`<span class="initial">${(a.name||"?")[0].toUpperCase()}</span>`;H.default.marker([a.lat,a.lon],{icon:H.default.divIcon({className:"person-icon",html:o,iconSize:[28,28],iconAnchor:[14,14]}),keyboard:!1,zIndexOffset:1e3}).bindTooltip(`${a.name} (${a.state})`).addTo(this._personLayer)}}}_renderGeoMarkers(){if(!this._map)return;let r=JSON.stringify(this.geoMarkers||[]);if(r!==this._geoSignature){this._geoSignature=r,this._geoFeedLayer.clearLayers();for(let a of this.geoMarkers||[])H.default.marker([a.lat,a.lon],{icon:H.default.divIcon({className:"geo-feed-icon",html:"",iconSize:[12,12],iconAnchor:[6,6]}),keyboard:!1}).bindTooltip(`${a.name} \u2014 ${a.source}`+(a.distance&&a.distance!=="unknown"?` (${a.distance} ${a.unit})`:"")).addTo(this._geoFeedLayer)}}_renderCapture(){if(this._captureLayer.clearLayers(),this.capture?.mode)this.setAttribute("data-capturing","");else{this.removeAttribute("data-capturing");return}let r=this._accentColor(),a=this.capture.points||[];for(let[o,l]of a)H.default.circleMarker([o,l],{radius:5,color:r,fillColor:r,fillOpacity:.9}).addTo(this._captureLayer);if(a.length>=2){let o=a.map(([l,u])=>[l,u]);this.capture.mode==="polygon"&&a.length>=3?H.default.polygon(o,{color:r,weight:2,dashArray:"4 4",fillOpacity:.1}).addTo(this._captureLayer):H.default.polyline(o,{color:r,weight:2,dashArray:"4 4"}).addTo(this._captureLayer)}}_focusEvent(r){let a=this._markersById.get(r);if(!a)return;let{layer:o,clustered:l}=a;l&&o.getLatLng?this._clusterGroup.zoomToShowLayer(o,()=>o.openPopup()):o.getBounds?(this._map.fitBounds(o.getBounds().pad(.3),{maxZoom:15}),o.openPopup()):o.getLatLng&&(this._map.panTo(o.getLatLng()),o.openPopup())}};et(Pe,"properties",{events:{attribute:!1},center:{attribute:!1},radiusKm:{attribute:!1},radiusEnabled:{attribute:!1},zones:{attribute:!1},persons:{attribute:!1},geoMarkers:{attribute:!1},capture:{attribute:!1},selectedId:{attribute:!1},dark:{type:Boolean,reflect:!0}}),et(Pe,"styles",[st`
      ${Yt(Ao)}
    `,st`
      ${Yt(Oo)}
    `,st`
    ...
    `]);customElements.define("chronotope-map-view",Pe);var Hs=["Mo","Di","Mi","Do","Fr","Sa","So"],ke=class extends Q{constructor(){super(),this.profiles=[],this.selectedProfileId="",this._profileName=""}updated(r){if(r.has("selectedProfileId")){let a=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this._profileName=a?a.name:""}}render(){let r=this.state;return I`
      <div class="groups">
        <div class="group">
          ...
          <div class="row">
            ...
            ${this.selectedProfileId?I`<button
                  class="ics-button"
        ...
        <div class="group">
          ...
          <div class="chips">
            ${(this.categories||[]).length===0?I`<span class="hint">Noch keine Kategorien</span>`:(this.categories||[]).map(a=>I`
                    <button
                      class="chip"
    ...
    `}_renderStats(){let r=this.stats;return r?I`
        ...
        <table>
          ...
          ${(r.sources||[]).map(a=>I`
              <tr>
                <td>${a.source}</td>
                <td>
                  ${a.events} Events${a.last_scraped?I`, zuletzt ${new Date(a.last_scraped).toLocaleDateString()}`:$}
                </td>
              </tr>
            `)}
        </table>
      </details>
    `:$}_patch(r){this.dispatchEvent(new CustomEvent("filters-changed",{detail:r}))}_renderDaySlider(r){if(!r.start||!r.end)return $;let a=new Date(r.start);a.setHours(0,0,0,0);let o=new Date(r.end),l=Math.min(Math.ceil((o-a)/864e5),60);if(l<2)return $;let u=p=>{if(!p)return 0;let x=new Date(`${p}T00:00:00`);return Math.round((x-a)/864e5)+1},c=p=>{if(!p)return"";let x=new Date(a.getTime()+(p-1)*864e5),y=P=>String(P).padStart(2,"0");return`${x.getFullYear()}-${y(x.getMonth()+1)}-${y(x.getDate())}`},_=u(r.dayFilter),g=r.dayFilter?new Date(`${r.dayFilter}T00:00:00`).toLocaleDateString(void 0,{weekday:"short",day:"2-digit",month:"2-digit"}):"Alle Tage";return I`
      <div class="row">
        <input
          type="range"
          min="0"
    ...
    `}_saveProfile(){let r=(this._profileName||"").trim();if(!r)return;let a=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this.dispatchEvent(new CustomEvent("profile-save",{detail:{name:r,id:a&&a.name===r?a.id:void 0}}))}_toggleCategory(r){let a=this.state.categories.includes(r)?this.state.categories.filter(o=>o!==r):[...this.state.categories,r];this._patch({categories:a})}_toggleWeekday(r){let a=this.state.weekdays.includes(r)?this.state.weekdays.filter(o=>o!==r):[...this.state.weekdays,r];this._patch({weekdays:a})}};et(ke,"properties",{state:{attribute:!1},categories:{attribute:!1},icsCopied:{attribute:!1},profiles:{attribute:!1},selectedProfileId:{attribute:!1},stats:{attribute:!1},_profileName:{state:!0}}),et(ke,"styles",st`
    :host {
      display: block;
      background: var(--card-background-color, #fff);
  ...
  `);customElements.define("chronotope-filter-bar",ke);var Ce=class extends Q{render(){let r=this.events||[];return r.length===0?I`<div class="empty">Keine Events für die aktuellen Filter.</div>`:I`${r.map(a=>this._renderItem(a))}`}_renderItem(r){let a=r.occurrences?.[0]?.[0]??r.start_time,o=r.occurrences?.[0]?.[1]??r.end_time;return I`
      <button
        class="item"
      ...
      >
        <div class="title-row">
          ...
          <span>
            ${r.distance_km!=null?I`<span class="distance">${this._formatDistance(r.distance_km)}</span>`:$}
            <button
        ...
        <div class="meta">
          ...
          ${r.recurrence?I`<span title=${r.recurrence}>🔁</span>`:$}
          ${r.category?I`<span class="badge">${r.category}</span>`:$}
          ${r.confidence?I`<span class="badge confidence-${r.confidence}">${r.confidence}</span>`:$}
          ${r.source_url?I`<span class="source">
                <a href=${r.source_url} target="_blank" rel="noopener noreferrer"
                  @click=${l=>l.stopPropagation()}
                  >${r.source_name||"Quelle"}</a
                >
              </span>`:$}
          ${r.visits?.length?I`<span
                class="visited"
                title=${r.visits.map(l=>`${l.person_id} (${new Date(l.last_seen).toLocaleDateString()})`).join(", ")}
                >✓ besucht</span
              >`:$}
        </div>
        ${r.address?I`<div class="address">${r.address}</div>`:$}
      </button>
    `}_flag(r,a,o){r.stopPropagation(),this.dispatchEvent(new CustomEvent("event-flag",{detail:{id:a.id,...o}}))}_renderWhen(r,a,o){if(r.time_precision==="approximate"){let l=r.schedule_text||this._formatRange(a,o);return I`<span class="fuzzy" title="Unpräzise Zeitangabe">~ ${l}</span>`}return I`<span>${this._formatRange(a,o)}</span>`}_formatDistance(r){return`${(r<10?r.toFixed(1):Math.round(r).toString()).replace(".",",")} km`}_formatRange(r,a){let o=this.locale||void 0,l=new Date(r),u=new Date(a),c=new Intl.DateTimeFormat(o,{weekday:"short",day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}),_=new Intl.DateTimeFormat(o,{hour:"2-digit",minute:"2-digit"});return l.toDateString()===u.toDateString()?`${c.format(l)} \u2013 ${_.format(u)}`:`${c.format(l)} \u2013 ${c.format(u)}`}};et(Ce,"properties",{events:{attribute:!1},selectedId:{attribute:!1},locale:{attribute:!1}}),et(Ce,"styles",st`
    :host {
      display: block;
      overflow-y: auto;
      background: var(--primary-background-color, #fafafa);
  ...
  `);customElements.define("chronotope-event-list",Ce);function Io(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Se=class extends Q{willUpdate(r){if(r.has("event")){let a=this.event||{};this._draft={id:a.id,title:a.title||"",category:a.category||"",start:Io(a.start_time),end:Io(a.end_time),address:a.address||"",lat:a.lat??"",lon:a.lon??"",recurrence:a.recurrence||"",time_precision:a.time_precision||"exact",schedule_text:a.schedule_text||"",source_name:a.source_name||"",source_url:a.source_url||"",raw_description:a.raw_description||"",geometry:a.geometry||"",favorite:!!a.favorite},this._error=null}}setCoords(r,a){this._draft={...this._draft,lat:Number(r.toFixed(6)),lon:Number(a.toFixed(6))}}setGeometry(r){this._draft={...this._draft,geometry:JSON.stringify(r)}}render(){let r=this._draft||{};return I`
      <form @submit=${this._save}>
        ...
        <label>
          ...
          <datalist id="categories">
            ${(this.categories||[]).map(a=>I`<option value=${a}></option>`)}
          </datalist>
        </label>
        <div class="row">
          <label>
        ...
        <div class="buttons">
          ...
          ${this.captureMode==="line"||this.captureMode==="polygon"?I`<button type="button" class="primary" @click=${this._finishCapture}>
                ✓ Zeichnung übernehmen
              </button>`:$}
        </div>
        ${this.captureMode?I`<div class="hint">
    ...
    `}_set(r){return a=>{this._draft={...this._draft,[r]:a.target.value}}}_requestCapture(r){let a=this.captureMode===r?null:r;this.dispatchEvent(new CustomEvent("capture-request",{detail:{mode:a}}))}_finishCapture(){this.dispatchEvent(new CustomEvent("capture-finish"))}_save(r){r.preventDefault();let a=this._draft;if(!a.start||!a.end){this._error="Beginn und Ende sind Pflichtfelder.";return}let o=null;if(a.geometry&&a.geometry.trim())try{o=JSON.parse(a.geometry)}catch{this._error="Geometrie ist kein g\xFCltiges JSON.";return}let l=a.lat!==""&&a.lat!=null,u=a.lon!==""&&a.lon!=null;if(l!==u){this._error="Lat und Lon nur gemeinsam angeben.";return}let c={id:a.id||void 0,title:a.title,category:a.category,start_time:new Date(a.start).toISOString(),end_time:new Date(a.end).toISOString(),address:a.address||null,lat:l?Number(a.lat):null,lon:u?Number(a.lon):null,recurrence:a.recurrence||null,time_precision:a.time_precision,schedule_text:a.schedule_text||null,source_name:a.source_name||null,source_url:a.source_url||null,raw_description:a.raw_description||null,geometry:o,favorite:a.favorite};this.dispatchEvent(new CustomEvent("editor-save",{detail:{event:c}}))}};et(Se,"properties",{event:{attribute:!1},categories:{attribute:!1},captureMode:{attribute:!1},_draft:{state:!0},_error:{state:!0}}),et(Se,"styles",st`
    :host {
      display: block;
      position: absolute;
      top: 0;
  ...
  `);customElements.define("chronotope-event-editor",Se);var Zo=(d,r)=>d.callWS({type:"chronotope/events/query",...r}),Bo=d=>d.callWS({type:"chronotope/categories"}),No=(d,r)=>d.callWS({type:"chronotope/ics_url",...r}),Do=(d,r)=>d.callWS({type:"chronotope/events/save",event:r}),Ro=(d,r)=>d.callWS({type:"chronotope/events/delete",event_id:r}),Fo=(d,r,a)=>d.callWS({type:"chronotope/events/flag",event_id:r,...a}),Ho=d=>d.callWS({type:"chronotope/profiles/list"}),Uo=(d,r)=>d.callWS({type:"chronotope/profiles/save",profile:r}),Go=(d,r)=>d.callWS({type:"chronotope/profiles/delete",profile_id:r}),Wo=d=>d.callWS({type:"chronotope/stats"});function Qe(d){let r={};return d.categories.length&&(r.categories=[...d.categories]),d.radiusEnabled&&d.center&&(r.center={lat:d.center.lat,lon:d.center.lon},r.radius_km=d.radiusKm),d.start&&(r.start=new Date(d.start).toISOString()),d.end&&(r.end=new Date(d.end).toISOString()),d.weekdays.length&&(r.weekdays=[...d.weekdays]),d.timeMode==="range"&&(d.timeFrom&&(r.time_from=d.timeFrom),d.timeTo&&(r.time_to=d.timeTo)),d.text&&(r.text=d.text),d.favoritesOnly&&(r.favorites_only=!0),r}var Us=250,Gs=new Set(["showZones","showPersons","showGeoFeeds","dayFilter"]);function qo(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Te=class extends Q{constructor(){super(),this._events=[],this._categories=[],this._selectedId=null,this._icsCopied=!1,this._error=null,this._profiles=[],this._selectedProfileId="",this._editing=null,this._capture=null,this._stats=null,this._filters={categories:[],radiusEnabled:!1,radiusKm:10,center:null,start:"",end:"",weekdays:[],timeMode:"allday",timeFrom:"",timeTo:"",text:"",favoritesOnly:!1,showZones:!0,showPersons:!0,showGeoFeeds:!0,dayFilter:""},this._initialized=!1}willUpdate(r){r.has("hass")&&this.hass&&!this._initialized&&(this._initialized=!0,this._filters={...this._filters,center:{lat:this.hass.config.latitude,lon:this.hass.config.longitude}},this._loadCategories(),this._loadProfiles(),this._loadStats(),this._runQuery())}async _loadStats(){try{this._stats=await Wo(this.hass)}catch(r){console.error("chronotope: loading stats failed",r)}}get _displayedEvents(){if(!this._filters.dayFilter)return this._events;let r=new Date(`${this._filters.dayFilter}T00:00:00`),a=new Date(r.getTime()+864e5);return this._events.filter(o=>(o.occurrences||[[o.start_time,o.end_time]]).some(([u,c])=>new Date(u)<a&&new Date(c)>r))}render(){let r=!!this.hass?.themes?.darkMode,a=this._displayedEvents;return I`
      <header>
        <h1>Chronotope</h1>
        <span class="count">
          ${a.length} ${a.length===1?"Event":"Events"}
        </span>
      ...
      <div class="content ${this.narrow?"narrow":""}">
        ...
        <chronotope-map-view
          ...
          @event-selected=${this._onEventSelected}
        ></chronotope-map-view>
        ${this._editing!==null?I`<chronotope-event-editor
              .event=${this._editing}
              .categories=${this._categories}
    ...
    `}_haZones(){let r=this.hass?.states||{},a=Object.values(r).filter(o=>o.entity_id.startsWith("zone.")).map(o=>({id:o.entity_id,name:o.attributes.friendly_name||o.entity_id,lat:o.attributes.latitude,lon:o.attributes.longitude,radius:o.attributes.radius??100,passive:!!o.attributes.passive,home:o.entity_id==="zone.home"})).filter(o=>o.lat!=null&&o.lon!=null);return!a.some(o=>o.home)&&this.hass?.config?.latitude!=null&&a.push({id:"home",name:"Zuhause",lat:this.hass.config.latitude,lon:this.hass.config.longitude,radius:100,passive:!1,home:!0}),a}_haPersons(){let r=this.hass?.states||{};return Object.values(r).filter(a=>a.entity_id.startsWith("person.")).map(a=>({id:a.entity_id,name:a.attributes.friendly_name||a.entity_id,lat:a.attributes.latitude,lon:a.attributes.longitude,picture:a.attributes.entity_picture||null,state:a.state})).filter(a=>a.lat!=null&&a.lon!=null)}_haGeoLocations(){let r=this.hass?.states||{};return Object.values(r).filter(a=>a.entity_id.startsWith("geo_location.")).map(a=>({id:a.entity_id,name:a.attributes.friendly_name||a.entity_id,lat:a.attributes.latitude,lon:a.attributes.longitude,source:a.attributes.source||"geo_location",distance:a.state,unit:a.attributes.unit_of_measurement||"km"})).filter(a=>a.lat!=null&&a.lon!=null)}_onFiltersChanged(r){this._filters={...this._filters,...r.detail},this._icsCopied=!1,Object.keys(r.detail).filter(o=>!Gs.has(o)).length&&this._scheduleQuery()}_onMapClick(r){let{lat:a,lon:o}=r.detail;if(this._capture?.mode==="point"){this._editorElement()?.setCoords(a,o),this._capture=null;return}if(this._capture?.mode){this._capture={...this._capture,points:[...this._capture.points,[a,o]]};return}this._filters={...this._filters,center:{lat:a,lon:o}},this._filters.radiusEnabled&&this._scheduleQuery()}_onEventSelected(r){this._selectedId=r.detail.id}_scheduleQuery(){clearTimeout(this._queryTimer),this._queryTimer=setTimeout(()=>this._runQuery(),Us)}async _loadCategories(){try{let r=await Bo(this.hass);this._categories=r.categories}catch(r){console.error("chronotope: loading categories failed",r)}}async _loadProfiles(){try{let r=await Ho(this.hass);this._profiles=r.profiles}catch(r){console.error("chronotope: loading profiles failed",r)}}async _runQuery(){if(this.hass)try{let r=await Zo(this.hass,Qe(this._filters));this._events=r.events,this._error=null,this._selectedId&&!this._events.some(a=>a.id===this._selectedId)&&(this._selectedId=null)}catch(r){this._error=`Abfrage fehlgeschlagen: ${r.message||r.code||r}`}}_onProfileSelected(r){this._selectedProfileId=r.detail.id;let a=this._profiles.find(o=>o.id===r.detail.id);a&&this._applyProfileFilters(a.filters||{})}_applyProfileFilters(r){let a=r.center||this._filters.center;this._filters={...this._filters,categories:r.categories||[],radiusEnabled:!!(r.center&&r.radius_km!=null),radiusKm:r.radius_km!=null?r.radius_km:this._filters.radiusKm,center:a,start:qo(r.start),end:qo(r.end),weekdays:r.weekdays||[],timeMode:r.time_from||r.time_to?"range":"allday",timeFrom:r.time_from||"",timeTo:r.time_to||"",text:r.text||"",favoritesOnly:!!r.favorites_only,dayFilter:""},this._scheduleQuery()}async _onProfileSave(r){try{let a=await Uo(this.hass,{id:r.detail.id,name:r.detail.name,filters:Qe(this._filters)});await this._loadProfiles(),this._selectedProfileId=a.profile.id,this._error=null}catch(a){this._error=`Profil speichern fehlgeschlagen: ${a.message||a.code||a}`}}async _onProfileDelete(r){try{await Go(this.hass,r.detail.id),this._selectedProfileId===r.detail.id&&(this._selectedProfileId=""),await this._loadProfiles()}catch(a){this._error=`Profil l\xF6schen fehlgeschlagen: ${a.message||a.code||a}`}}_editorElement(){return this.renderRoot.querySelector("chronotope-event-editor")}_onNewEvent(){let r=new Date;r.setMinutes(0,0,0);let a=new Date(r.getTime()+2*36e5);this._editing={start_time:r.toISOString(),end_time:a.toISOString()},this._capture=null}_onEventEdit(r){let a=this._events.find(o=>o.id===r.detail.id);a&&(this._editing=a,this._capture=null)}async _onEditorSave(r){try{await Do(this.hass,r.detail.event),this._editing=null,this._capture=null,this._error=null,await this._runQuery(),await this._loadCategories()}catch(a){this._error=`Speichern fehlgeschlagen: ${a.message||a.code||a}`}}async _onEditorDelete(r){try{await Ro(this.hass,r.detail.id),this._editing=null,this._capture=null,await this._runQuery()}catch(a){this._error=`L\xF6schen fehlgeschlagen: ${a.message||a.code||a}`}}_onEditorCancel(){this._editing=null,this._capture=null}_onCaptureRequest(r){let a=r.detail.mode;this._capture=a?{mode:a,points:[]}:null}_onCaptureFinish(){let r=this._capture;if(r){if(r.mode==="line"&&r.points.length>=2)this._editorElement()?.setGeometry({type:"LineString",coordinates:r.points.map(([a,o])=>[o,a])});else if(r.mode==="polygon"&&r.points.length>=3){let a=r.points.map(([o,l])=>[l,o]);a.push(a[0]),this._editorElement()?.setGeometry({type:"Polygon",coordinates:[a]})}else{this._error="Zu wenige Punkte f\xFCr die Zeichnung.";return}this._capture=null,this._error=null}}async _onEventFlag(r){let{id:a,...o}=r.detail;try{await Fo(this.hass,a,o),await this._runQuery()}catch(l){this._error=`Aktion fehlgeschlagen: ${l.message||l.code||l}`}}async _onIcsRequested(){try{let r=this._selectedProfileId?{profile_id:this._selectedProfileId}:Qe(this._filters),a=await No(this.hass,r);await navigator.clipboard.writeText(a.url),this._icsCopied=!0,setTimeout(()=>{this._icsCopied=!1},3e3)}catch(r){this._error=`ICS-URL konnte nicht kopiert werden: ${r.message||r.code||r}`}}};et(Te,"properties",{hass:{attribute:!1},narrow:{attribute:!1},route:{attribute:!1},panel:{attribute:!1},_events:{state:!0},_categories:{state:!0},_filters:{state:!0},_selectedId:{state:!0},_icsCopied:{state:!0},_error:{state:!0},_profiles:{state:!0},_selectedProfileId:{state:!0},_editing:{state:!0},_capture:{state:!0},_stats:{state:!0}}),et(Te,"styles",st`
    :host {
      display: flex;
      flex-direction: column;
      height: 100vh;
