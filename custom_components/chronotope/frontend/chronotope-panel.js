...
.marker-cluster span {
    ...
    `]);customElements.define("chronotope-map-view",ke);var Se=class extends tt{constructor(){super(),this.profiles=[],this.selectedProfileId="",this.collapsed=!1,this.narrow=!1,this._profileName=""}_activeFilterCount(){let r=this.state||{},a=0;return r.categories?.length&&(a+=1),r.radiusEnabled&&(a+=1),(r.start||r.end)&&(a+=1),r.weekdays?.length&&(a+=1),r.timeMode==="range"&&(r.timeFrom||r.timeTo)&&(a+=1),r.text&&(a+=1),r.favoritesOnly&&(a+=1),a}updated(r){if(r.has("selectedProfileId")){let a=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this._profileName=a?a.name:""}}render(){let r=this.state,a=this._activeFilterCount();return A`
      <button
        class="toggle-row"
        title=${this.collapsed?w("filters.show"):w("filters.hide")}
        @click=${()=>this.dispatchEvent(new CustomEvent("toggle-collapsed"))}
      >
        <span>
          ${w("filters.label")}
          ${a?A` <span class="active-count">· ${w("filters.active",{n:a})}</span>`:O}
        </span>
        ${this.collapsed?qo:jo}
      </button>
      ${this.collapsed?O:this._renderGroups(r)}
    `}_renderGroups(r){return A`
      <div class="groups">
        <div class="group">
          ...
          <div class="row">
            ...
            ${this.selectedProfileId?A`<button
                  class="ics-button"
        ...
        <div class="group">
          ...
          <div class="row">
            ...
            <label class="row" style="gap:4px">
              <input
                type="checkbox"
                .checked=${r.showHidden}
                @change=${a=>this._patch({showHidden:a.target.checked})}
              />
              ${w("search.showHidden")}
            </label>
          </div>
        ...
        <div class="group">
          ...
          <div class="chips">
            ${(this.categories||[]).length===0?A`<span class="hint">${w("category.none")}</span>`:(this.categories||[]).map(a=>A`
                    <button
                      class="chip"
    ...
    `}_renderStats(){let r=this.stats;return r?A`
        ...
        <table>
          ...
          ${(r.sources||[]).map(a=>A`
              <tr>
                <td>${a.source}</td>
                <td>
                  ${a.events} ${w("stats.events")}${a.last_scraped?A`, ${w("stats.last")}
                      ${new Date(a.last_scraped).toLocaleDateString()}`:O}
                </td>
              </tr>
            `)}
        </table>
      </details>
    `:O}_patch(r){this.dispatchEvent(new CustomEvent("filters-changed",{detail:r}))}_renderDaySlider(r){if(!r.start||!r.end)return O;let a=new Date(r.start);a.setHours(0,0,0,0);let o=new Date(r.end),l=Math.min(Math.ceil((o-a)/864e5),60);if(l<2)return O;let u=p=>{if(!p)return 0;let x=new Date(`${p}T00:00:00`);return Math.round((x-a)/864e5)+1},c=p=>{if(!p)return"";let x=new Date(a.getTime()+(p-1)*864e5),y=C=>String(C).padStart(2,"0");return`${x.getFullYear()}-${y(x.getMonth()+1)}-${y(x.getDate())}`},_=u(r.dayFilter),g=r.dayFilter?new Date(`${r.dayFilter}T00:00:00`).toLocaleDateString(void 0,{weekday:"short",day:"2-digit",month:"2-digit"}):w("window.allDays");return A`
      <div class="row">
        <input
          type="range"
          min="0"
  ...
  `);customElements.define("chronotope-filter-bar",Se);var Te=class extends tt{render(){let r=this.events||[];return r.length===0?A`<div class="empty">${w("list.empty")}</div>`:A`${r.map(a=>this._renderItem(a))}`}_renderItem(r){let a=r.occurrences?.[0]?.[0]??r.start_time,o=r.occurrences?.[0]?.[1]??r.end_time;return A`
      <button
        class="item ${r.hidden?"is-hidden":""}"
      ...
      >
        <div class="title-row">
          ...
          <span>
            ${r.distance_km!=null?A`<span class="distance">${this._formatDistance(r.distance_km)}</span>`:O}
            <button
            ...
            <button
              class="icon-btn"
              title=${r.hidden?w("list.unhide"):w("list.hide")}
              @click=${l=>this._flag(l,r,{hidden:!r.hidden})}
            >
              ${r.hidden?Wo:Go}
            </button>
          </span>
        ...
        <div class="meta">
          ...
          ${r.recurrence?A`<span title=${r.recurrence}>${Vo}</span>`:O}
          ${r.hidden?A`<span class="badge">${w("list.hidden")}</span>`:O}
          ${r.category?A`<span class="badge">${r.category}</span>`:O}
          ${r.confidence?A`<span class="badge confidence-${r.confidence}">${r.confidence}</span>`:O}
          ${r.source_url?A`<span class="source">
                <a href=${r.source_url} target="_blank" rel="noopener noreferrer"
                  @click=${l=>l.stopPropagation()}
                  >${r.source_name||w("list.source")}</a
                >
              </span>`:O}
          ${r.visits?.length?A`<span
                class="visited"
                title=${r.visits.map(l=>`${l.person_id} (${new Date(l.last_seen).toLocaleDateString()})`).join(", ")}
                >${w("list.visited")}</span
              >`:O}
        </div>
        ${r.address?A`<div class="address">${r.address}</div>`:O}
      </button>
    `}_flag(r,a,o){r.stopPropagation(),this.dispatchEvent(new CustomEvent("event-flag",{detail:{id:a.id,...o}}))}_renderWhen(r,a,o){if(r.time_precision==="approximate"){let l=r.schedule_text||this._formatRange(a,o);return A`<span class="fuzzy" title=${w("list.fuzzy")}>~ ${l}</span>`}return A`<span>${this._formatRange(a,o)}</span>`}_formatDistance(r){return`${(r<10?r.toFixed(1):Math.round(r).toString()).replace(".",",")} km`}_formatRange(r,a){let o=this.locale||void 0,l=new Date(r),u=new Date(a),c=new Intl.DateTimeFormat(o,{weekday:"short",day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}),_=new Intl.DateTimeFormat(o,{hour:"2-digit",minute:"2-digit"});return l.toDateString()===u.toDateString()?`${c.format(l)} \u2013 ${_.format(u)}`:`${c.format(l)} \u2013 ${c.format(u)}`}};it(Te,"properties",{events:{attribute:!1},selectedId:{attribute:!1},locale:{attribute:!1}}),it(Te,"styles",at`
    :host {
      display: block;
      overflow-y: auto;
      background: var(--primary-background-color, #fafafa);
  ...
  `);customElements.define("chronotope-event-list",Te);function Qo(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Me=class extends tt{updated(r){r.has("captureMode")&&this.toggleAttribute("capturing",!!this.captureMode)}willUpdate(r){if(r.has("event")){let a=this.event||{};this._draft={id:a.id,title:a.title||"",category:a.category||"",start:Qo(a.start_time),end:Qo(a.end_time),address:a.address||"",lat:a.lat??"",lon:a.lon??"",recurrence:a.recurrence||"",time_precision:a.time_precision||"exact",schedule_text:a.schedule_text||"",source_name:a.source_name||"",source_url:a.source_url||"",raw_description:a.raw_description||"",geometry:a.geometry||"",favorite:!!a.favorite},this._error=null}}setCoords(r,a){this._draft={...this._draft,lat:Number(r.toFixed(6)),lon:Number(a.toFixed(6))}}setGeometry(r){this._draft={...this._draft,geometry:JSON.stringify(r)}}render(){let r=this._draft||{};return A`
      <div class="grip"></div>
      <div class="head">
      ...
      <form @submit=${this._save}>
        ${this._error?A`<div class="error">${this._error}</div>`:O}
        <label>
          ${w("editor.title")}
        ...
        <label>
          ...
          <datalist id="categories">
            ${(this.categories||[]).map(a=>A`<option value=${a}></option>`)}
          </datalist>
        </label>
        <div class="row">
          <label>
        ...
        <div class="buttons">
          ...
          ${this.captureMode==="line"||this.captureMode==="polygon"?A`<button type="button" class="primary" @click=${this._finishCapture}>
                ${w("editor.applyDrawing")}
              </button>`:O}
        </div>
        ${this.captureMode?A`<div class="hint">
  ...
  `);customElements.define("chronotope-event-editor",Me);var tr=(d,r)=>d.callWS({type:"chronotope/events/query",...r}),er=d=>d.callWS({type:"chronotope/categories"}),ir=(d,r)=>d.callWS({type:"chronotope/ics_url",...r}),nr=(d,r)=>d.callWS({type:"chronotope/events/save",event:r}),or=(d,r)=>d.callWS({type:"chronotope/events/delete",event_id:r}),rr=(d,r,a)=>d.callWS({type:"chronotope/events/flag",event_id:r,...a}),sr=d=>d.callWS({type:"chronotope/profiles/list"}),ar=(d,r)=>d.callWS({type:"chronotope/profiles/save",profile:r}),lr=(d,r)=>d.callWS({type:"chronotope/profiles/delete",profile_id:r}),hr=d=>d.callWS({type:"chronotope/stats"});function ii(d){let r={};return d.categories.length&&(r.categories=[...d.categories]),d.radiusEnabled&&d.center&&(r.center={lat:d.center.lat,lon:d.center.lon},r.radius_km=d.radiusKm),d.start&&(r.start=new Date(d.start).toISOString()),d.end&&(r.end=new Date(d.end).toISOString()),d.weekdays.length&&(r.weekdays=[...d.weekdays]),d.timeMode==="range"&&(d.timeFrom&&(r.time_from=d.timeFrom),d.timeTo&&(r.time_to=d.timeTo)),d.text&&(r.text=d.text),d.favoritesOnly&&(r.favorites_only=!0),d.showHidden&&(r.include_hidden=!0),r}var sa=250,aa=new Set(["showZones","showPersons","showGeoFeeds","dayFilter","filtersCollapsed"]),Yi="chronotope-panel-state-v1";function Ji(){return{categories:[],radiusEnabled:!1,radiusKm:10,center:null,start:"",end:"",weekdays:[],timeMode:"allday",timeFrom:"",timeTo:"",text:"",favoritesOnly:!1,showHidden:!1,showZones:!0,showPersons:!0,showGeoFeeds:!0,dayFilter:"",filtersCollapsed:!1}}function ur(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Ee=class extends tt{constructor(){super(),this._events=[],this._categories=[],this._selectedId=null,this._icsCopied=!1,this._error=null,this._profiles=[],this._selectedProfileId="",this._editing=null,this._capture=null,this._stats=null,this._filters=Ji(),this._initialized=!1}willUpdate(r){if(r.has("hass")&&this.hass&&Ro(this.hass.locale?.language||this.hass.language),r.has("hass")&&this.hass&&!this._initialized){this._initialized=!0;let a=this._loadPersistedState();this._filters={...Ji(),...a?.filters||{},center:a?.filters?.center||this._homeCenter(),filtersCollapsed:a?.filters?.filtersCollapsed??!!this.narrow},this._selectedProfileId=a?.selectedProfileId||"",this._loadCategories(),this._loadProfiles(),this._loadStats(),this._runQuery()}}updated(r){if(this._initialized&&(r.has("_filters")||r.has("_selectedProfileId")))try{window.localStorage.setItem(Yi,JSON.stringify({filters:this._filters,selectedProfileId:this._selectedProfileId}))}catch{}}_homeCenter(){return{lat:this.hass.config.latitude,lon:this.hass.config.longitude}}_loadPersistedState(){try{let r=window.localStorage.getItem(Yi);return r?JSON.parse(r):null}catch(r){return console.warn("chronotope: persisted panel state unreadable",r),null}}_onResetView(){try{window.localStorage.removeItem(Yi)}catch{}this._filters={...Ji(),center:this._homeCenter()},this._selectedProfileId="",this._icsCopied=!1,this._error=null,this._scheduleQuery()}async _loadStats(){try{this._stats=await hr(this.hass)}catch(r){console.error("chronotope: loading stats failed",r)}}get _displayedEvents(){if(!this._filters.dayFilter)return this._events;let r=new Date(`${this._filters.dayFilter}T00:00:00`),a=new Date(r.getTime()+864e5);return this._events.filter(o=>(o.occurrences||[[o.start_time,o.end_time]]).some(([u,c])=>new Date(u)<a&&new Date(c)>r))}render(){let r=!!this.hass?.themes?.darkMode,a=this._displayedEvents;return A`
      <header>
        <h1>Chronotope</h1>
        <span class="count">
      ...
      <div class="content ${this.narrow?"narrow":""}">
        ...
        <chronotope-map-view
          ...
          @event-selected=${this._onEventSelected}
        ></chronotope-map-view>
        ${this._editing!==null?A`<chronotope-event-editor
              .event=${this._editing}
              .categories=${this._categories}
    ...
    `}_haZones(){let r=this.hass?.states||{},a=Object.values(r).filter(o=>o.entity_id.startsWith("zone.")).map(o=>({id:o.entity_id,name:o.attributes.friendly_name||o.entity_id,lat:o.attributes.latitude,lon:o.attributes.longitude,radius:o.attributes.radius??100,passive:!!o.attributes.passive,home:o.entity_id==="zone.home"})).filter(o=>o.lat!=null&&o.lon!=null);return!a.some(o=>o.home)&&this.hass?.config?.latitude!=null&&a.push({id:"home",name:w("panel.home"),lat:this.hass.config.latitude,lon:this.hass.config.longitude,radius:100,passive:!1,home:!0}),a}_haPersons(){let r=this.hass?.states||{};return Object.values(r).filter(a=>a.entity_id.startsWith("person.")).map(a=>({id:a.entity_id,name:a.attributes.friendly_name||a.entity_id,lat:a.attributes.latitude,lon:a.attributes.longitude,picture:a.attributes.entity_picture||null,state:a.state})).filter(a=>a.lat!=null&&a.lon!=null)}_haGeoLocations(){let r=this.hass?.states||{};return Object.values(r).filter(a=>a.entity_id.startsWith("geo_location.")).map(a=>({id:a.entity_id,name:a.attributes.friendly_name||a.entity_id,lat:a.attributes.latitude,lon:a.attributes.longitude,source:a.attributes.source||"geo_location",distance:a.state,unit:a.attributes.unit_of_measurement||"km"})).filter(a=>a.lat!=null&&a.lon!=null)}_onToggleFilters(){this._filters={...this._filters,filtersCollapsed:!this._filters.filtersCollapsed}}_onFiltersChanged(r){this._filters={...this._filters,...r.detail},this._icsCopied=!1,Object.keys(r.detail).filter(o=>!aa.has(o)).length&&this._scheduleQuery()}_onMapClick(r){let{lat:a,lon:o}=r.detail;if(this._capture?.mode==="point"){this._editorElement()?.setCoords(a,o),this._capture=null;return}if(this._capture?.mode){this._capture={...this._capture,points:[...this._capture.points,[a,o]]};return}this._filters={...this._filters,center:{lat:a,lon:o}},this._filters.radiusEnabled&&this._scheduleQuery()}_onEventSelected(r){this._selectedId=r.detail.id}_scheduleQuery(){clearTimeout(this._queryTimer),this._queryTimer=setTimeout(()=>this._runQuery(),sa)}async _loadCategories(){try{let r=await er(this.hass);this._categories=r.categories}catch(r){console.error("chronotope: loading categories failed",r)}}async _loadProfiles(){try{let r=await sr(this.hass);this._profiles=r.profiles}catch(r){console.error("chronotope: loading profiles failed",r)}}async _runQuery(){if(this.hass)try{let r=await tr(this.hass,ii(this._filters));this._events=r.events,this._error=null,this._selectedId&&!this._events.some(a=>a.id===this._selectedId)&&(this._selectedId=null)}catch(r){this._error=w("error.query",{msg:r.message||r.code||r})}}_onProfileSelected(r){this._selectedProfileId=r.detail.id;let a=this._profiles.find(o=>o.id===r.detail.id);a&&this._applyProfileFilters(a.filters||{})}_applyProfileFilters(r){let a=r.center||this._filters.center;this._filters={...this._filters,categories:r.categories||[],radiusEnabled:!!(r.center&&r.radius_km!=null),radiusKm:r.radius_km!=null?r.radius_km:this._filters.radiusKm,center:a,start:ur(r.start),end:ur(r.end),weekdays:r.weekdays||[],timeMode:r.time_from||r.time_to?"range":"allday",timeFrom:r.time_from||"",timeTo:r.time_to||"",text:r.text||"",favoritesOnly:!!r.favorites_only,dayFilter:""},this._scheduleQuery()}async _onProfileSave(r){try{let a=await ar(this.hass,{id:r.detail.id,name:r.detail.name,filters:ii(this._filters)});await this._loadProfiles(),this._selectedProfileId=a.profile.id,this._error=null}catch(a){this._error=w("error.profileSave",{msg:a.message||a.code||a})}}async _onProfileDelete(r){try{await lr(this.hass,r.detail.id),this._selectedProfileId===r.detail.id&&(this._selectedProfileId=""),await this._loadProfiles()}catch(a){this._error=w("error.profileDelete",{msg:a.message||a.code||a})}}_editorElement(){return this.renderRoot.querySelector("chronotope-event-editor")}_onNewEvent(){let r=new Date;r.setMinutes(0,0,0);let a=new Date(r.getTime()+2*36e5);this._editing={start_time:r.toISOString(),end_time:a.toISOString()},this._capture=null}_onEventEdit(r){let a=this._events.find(o=>o.id===r.detail.id);a&&(this._editing=a,this._capture=null)}async _onEditorSave(r){try{await nr(this.hass,r.detail.event),this._editing=null,this._capture=null,this._error=null,await this._runQuery(),await this._loadCategories()}catch(a){this._error=w("error.save",{msg:a.message||a.code||a})}}async _onEditorDelete(r){try{await or(this.hass,r.detail.id),this._editing=null,this._capture=null,await this._runQuery()}catch(a){this._error=w("error.delete",{msg:a.message||a.code||a})}}_onEditorCancel(){this._editing=null,this._capture=null}_onCaptureRequest(r){let a=r.detail.mode;this._capture=a?{mode:a,points:[]}:null}_onCaptureFinish(){let r=this._capture;if(r){if(r.mode==="line"&&r.points.length>=2)this._editorElement()?.setGeometry({type:"LineString",coordinates:r.points.map(([a,o])=>[o,a])});else if(r.mode==="polygon"&&r.points.length>=3){let a=r.points.map(([o,l])=>[l,o]);a.push(a[0]),this._editorElement()?.setGeometry({type:"Polygon",coordinates:[a]})}else{this._error=w("error.capture");return}this._capture=null,this._error=null}}async _onEventFlag(r){let{id:a,...o}=r.detail;try{await rr(this.hass,a,o),await this._runQuery()}catch(l){this._error=w("error.action",{msg:l.message||l.code||l})}}async _onIcsRequested(){try{let r=this._selectedProfileId?{profile_id:this._selectedProfileId}:ii(this._filters),a=await ir(this.hass,r);await navigator.clipboard.writeText(a.url),this._icsCopied=!0,setTimeout(()=>{this._icsCopied=!1},3e3)}catch(r){this._error=w("error.ics",{msg:r.message||r.code||r})}}};it(Ee,"properties",{hass:{attribute:!1},narrow:{attribute:!1},route:{attribute:!1},panel:{attribute:!1},_events:{state:!0},_categories:{state:!0},_filters:{state:!0},_selectedId:{state:!0},_icsCopied:{state:!0},_error:{state:!0},_profiles:{state:!0},_selectedProfileId:{state:!0},_editing:{state:!0},_capture:{state:!0},_stats:{state:!0}}),it(Ee,"styles",at`
    :host {
      display: flex;
      flex-direction: column;
      height: 100vh;
