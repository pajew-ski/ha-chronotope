...
.marker-cluster span {
    ...
    `]);customElements.define("chronotope-map-view",ke);var Se=class extends tt{constructor(){super(),this.profiles=[],this.selectedProfileId="",this.collapsed=!1,this.narrow=!1,this._profileName=""}_activeFilterCount(){let r=this.state||{},a=0;return r.categories?.length&&(a+=1),r.radiusEnabled&&(a+=1),(r.start||r.end)&&(a+=1),r.weekdays?.length&&(a+=1),r.timeMode==="range"&&(r.timeFrom||r.timeTo)&&(a+=1),r.text&&(a+=1),r.favoritesOnly&&(a+=1),a}updated(r){if(r.has("selectedProfileId")){let a=(this.profiles||[]).find(o=>o.id===this.selectedProfileId);this._profileName=a?a.name:""}}render(){let r=this.state,a=this._activeFilterCount();return I`
      ...
      >
        <span>
          ${w("filters.label")}
          ${a?I` <span class="active-count">· ${w("filters.active",{n:a})}</span>`:A}
        </span>
        ${this.collapsed?Vo:qo}
      </button>
      ${this.collapsed?A:this._renderGroups(r)}
    `}_renderGroups(r){return I`
      <div class="groups">
        <div class="group">
        ...
        <div class="group">
          ...
          <div class="chips">
            ${(this.categories||[]).length===0?I`<span class="hint">${w("category.none")}</span>`:(this.categories||[]).map(a=>I`
                    <button
                      class="chip"
        ...
        <table>
          ...
          ${(r.sources||[]).map(a=>I`
              <tr>
                <td>${a.source}</td>
                <td>
                  ${a.events} ${w("stats.events")}${a.last_scraped?I`, ${w("stats.last")}
                      ${new Date(a.last_scraped).toLocaleDateString()}`:A}
                </td>
              </tr>
            `)}
        </table>
      </details>
    `:A}_patch(r){this.dispatchEvent(new CustomEvent("filters-changed",{detail:r}))}_renderDaySlider(r){if(!r.start||!r.end)return A;let a=new Date(r.start);a.setHours(0,0,0,0);let o=new Date(r.end),l=Math.min(Math.ceil((o-a)/864e5),60);if(l<2)return A;let u=p=>{if(!p)return 0;let x=new Date(`${p}T00:00:00`);return Math.round((x-a)/864e5)+1},c=p=>{if(!p)return"";let x=new Date(a.getTime()+(p-1)*864e5),y=C=>String(C).padStart(2,"0");return`${x.getFullYear()}-${y(x.getMonth()+1)}-${y(x.getDate())}`},_=u(r.dayFilter),g=r.dayFilter?new Date(`${r.dayFilter}T00:00:00`).toLocaleDateString(void 0,{weekday:"short",day:"2-digit",month:"2-digit"}):w("window.allDays");return I`
      <div class="row">
        <input
          type="range"
          min="0"
  ...
  `);customElements.define("chronotope-filter-bar",Se);var Te=class extends tt{render(){let r=this.events||[];return r.length===0?I`<div class="empty">${w("list.empty")}</div>`:I`${r.map(a=>this._renderItem(a))}`}_renderItem(r){let a=r.occurrences?.[0]?.[0]??r.start_time,o=r.occurrences?.[0]?.[1]??r.end_time;return I`
      <button
        class="item"
      ...
      >
        <div class="title-row">
          ...
          <span>
            ${r.distance_km!=null?I`<span class="distance">${this._formatDistance(r.distance_km)}</span>`:A}
            <button
        ...
        <div class="meta">
          ...
          ${r.recurrence?I`<span title=${r.recurrence}>${Wo}</span>`:A}
          ${r.category?I`<span class="badge">${r.category}</span>`:A}
          ${r.confidence?I`<span class="badge confidence-${r.confidence}">${r.confidence}</span>`:A}
  ...
  `);customElements.define("chronotope-event-list",Te);function Xo(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Me=class extends tt{updated(r){r.has("captureMode")&&this.toggleAttribute("capturing",!!this.captureMode)}willUpdate(r){if(r.has("event")){let a=this.event||{};this._draft={id:a.id,title:a.title||"",category:a.category||"",start:Xo(a.start_time),end:Xo(a.end_time),address:a.address||"",lat:a.lat??"",lon:a.lon??"",recurrence:a.recurrence||"",time_precision:a.time_precision||"exact",schedule_text:a.schedule_text||"",source_name:a.source_name||"",source_url:a.source_url||"",raw_description:a.raw_description||"",geometry:a.geometry||"",favorite:!!a.favorite},this._error=null}}setCoords(r,a){this._draft={...this._draft,lat:Number(r.toFixed(6)),lon:Number(a.toFixed(6))}}setGeometry(r){this._draft={...this._draft,geometry:JSON.stringify(r)}}render(){let r=this._draft||{};return I`
      <div class="grip"></div>
      <div class="head">
  ...
  `);customElements.define("chronotope-event-editor",Me);var Qo=(d,r)=>d.callWS({type:"chronotope/events/query",...r}),tr=d=>d.callWS({type:"chronotope/categories"}),er=(d,r)=>d.callWS({type:"chronotope/ics_url",...r}),ir=(d,r)=>d.callWS({type:"chronotope/events/save",event:r}),nr=(d,r)=>d.callWS({type:"chronotope/events/delete",event_id:r}),or=(d,r,a)=>d.callWS({type:"chronotope/events/flag",event_id:r,...a}),rr=d=>d.callWS({type:"chronotope/profiles/list"}),sr=(d,r)=>d.callWS({type:"chronotope/profiles/save",profile:r}),ar=(d,r)=>d.callWS({type:"chronotope/profiles/delete",profile_id:r}),lr=d=>d.callWS({type:"chronotope/stats"});function ii(d){let r={};return d.categories.length&&(r.categories=[...d.categories]),d.radiusEnabled&&d.center&&(r.center={lat:d.center.lat,lon:d.center.lon},r.radius_km=d.radiusKm),d.start&&(r.start=new Date(d.start).toISOString()),d.end&&(r.end=new Date(d.end).toISOString()),d.weekdays.length&&(r.weekdays=[...d.weekdays]),d.timeMode==="range"&&(d.timeFrom&&(r.time_from=d.timeFrom),d.timeTo&&(r.time_to=d.timeTo)),d.text&&(r.text=d.text),d.favoritesOnly&&(r.favorites_only=!0),r}var ra=250,sa=new Set(["showZones","showPersons","showGeoFeeds","dayFilter","filtersCollapsed"]),Yi="chronotope-panel-state-v1";function Ji(){return{categories:[],radiusEnabled:!1,radiusKm:10,center:null,start:"",end:"",weekdays:[],timeMode:"allday",timeFrom:"",timeTo:"",text:"",favoritesOnly:!1,showZones:!0,showPersons:!0,showGeoFeeds:!0,dayFilter:"",filtersCollapsed:!1}}function hr(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Ee=class extends tt{constructor(){super(),this._events=[],this._categories=[],this._selectedId=null,this._icsCopied=!1,this._error=null,this._profiles=[],this._selectedProfileId="",this._editing=null,this._capture=null,this._stats=null,this._filters=Ji(),this._initialized=!1}willUpdate(r){if(r.has("hass")&&this.hass&&Ro(this.hass.locale?.language||this.hass.language),r.has("hass")&&this.hass&&!this._initialized){this._initialized=!0;let a=this._loadPersistedState();this._filters={...Ji(),...a?.filters||{},center:a?.filters?.center||this._homeCenter(),filtersCollapsed:a?.filters?.filtersCollapsed??!!this.narrow},this._selectedProfileId=a?.selectedProfileId||"",this._loadCategories(),this._loadProfiles(),this._loadStats(),this._runQuery()}}updated(r){if(this._initialized&&(r.has("_filters")||r.has("_selectedProfileId")))try{window.localStorage.setItem(Yi,JSON.stringify({filters:this._filters,selectedProfileId:this._selectedProfileId}))}catch{}}_homeCenter(){return{lat:this.hass.config.latitude,lon:this.hass.config.longitude}}_loadPersistedState(){try{let r=window.localStorage.getItem(Yi);return r?JSON.parse(r):null}catch(r){return console.warn("chronotope: persisted panel state unreadable",r),null}}_onResetView(){try{window.localStorage.removeItem(Yi)}catch{}this._filters={...Ji(),center:this._homeCenter()},this._selectedProfileId="",this._icsCopied=!1,this._error=null,this._scheduleQuery()}async _loadStats(){try{this._stats=await lr(this.hass)}catch(r){console.error("chronotope: loading stats failed",r)}}get _displayedEvents(){if(!this._filters.dayFilter)return this._events;let r=new Date(`${this._filters.dayFilter}T00:00:00`),a=new Date(r.getTime()+864e5);return this._events.filter(o=>(o.occurrences||[[o.start_time,o.end_time]]).some(([u,c])=>new Date(u)<a&&new Date(c)>r))}render(){let r=!!this.hass?.themes?.darkMode,a=this._displayedEvents;return I`
      <header>
        <h1>Chronotope</h1>
        <span class="count">
