...
.marker-cluster span {
  ...
  `);customElements.define("chronotope-filter-bar",Se);var Te=class extends tt{render(){let r=this.events||[];return r.length===0?Z`<div class="empty">${w("list.empty")}</div>`:Z`${r.map(a=>this._renderItem(a))}`}_renderItem(r){let a=r.occurrences?.[0]?.[0]??r.start_time,o=r.occurrences?.[0]?.[1]??r.end_time;return Z`
      ...
      >
        ...
        <div class="meta">
          ...
          ${r.recurrence?Z`<span title=${r.recurrence}>${Uo}</span>`:I}
  ...
  `);customElements.define("chronotope-event-editor",Me);var jo=(d,r)=>d.callWS({type:"chronotope/events/query",...r}),Ko=d=>d.callWS({type:"chronotope/categories"}),Yo=(d,r)=>d.callWS({type:"chronotope/ics_url",...r}),Jo=(d,r)=>d.callWS({type:"chronotope/events/save",event:r}),Xo=(d,r)=>d.callWS({type:"chronotope/events/delete",event_id:r}),Qo=(d,r,a)=>d.callWS({type:"chronotope/events/flag",event_id:r,...a}),tr=d=>d.callWS({type:"chronotope/profiles/list"}),er=(d,r)=>d.callWS({type:"chronotope/profiles/save",profile:r}),ir=(d,r)=>d.callWS({type:"chronotope/profiles/delete",profile_id:r}),nr=d=>d.callWS({type:"chronotope/stats"});function ii(d){let r={};return d.categories.length&&(r.categories=[...d.categories]),d.radiusEnabled&&d.center&&(r.center={lat:d.center.lat,lon:d.center.lon},r.radius_km=d.radiusKm),d.start&&(r.start=new Date(d.start).toISOString()),d.end&&(r.end=new Date(d.end).toISOString()),d.weekdays.length&&(r.weekdays=[...d.weekdays]),d.timeMode==="range"&&(d.timeFrom&&(r.time_from=d.timeFrom),d.timeTo&&(r.time_to=d.timeTo)),d.text&&(r.text=d.text),d.favoritesOnly&&(r.favorites_only=!0),r}var ta=250,ea=new Set(["showZones","showPersons","showGeoFeeds","dayFilter"]);function or(d){if(!d)return"";let r=new Date(d);if(Number.isNaN(r.getTime()))return"";let a=o=>String(o).padStart(2,"0");return`${r.getFullYear()}-${a(r.getMonth()+1)}-${a(r.getDate())}T${a(r.getHours())}:${a(r.getMinutes())}`}var Ee=class extends tt{constructor(){super(),this._events=[],this._categories=[],this._selectedId=null,this._icsCopied=!1,this._error=null,this._profiles=[],this._selectedProfileId="",this._editing=null,this._capture=null,this._stats=null,this._filters={categories:[],radiusEnabled:!1,radiusKm:10,center:null,start:"",end:"",weekdays:[],timeMode:"allday",timeFrom:"",timeTo:"",text:"",favoritesOnly:!1,showZones:!0,showPersons:!0,showGeoFeeds:!0,dayFilter:""},this._initialized=!1}willUpdate(r){r.has("hass")&&this.hass&&No(this.hass.locale?.language||this.hass.language),r.has("hass")&&this.hass&&!this._initialized&&(this._initialized=!0,this._filters={...this._filters,center:{lat:this.hass.config.latitude,lon:this.hass.config.longitude}},this._loadCategories(),this._loadProfiles(),this._loadStats(),this._runQuery())}async _loadStats(){try{this._stats=await nr(this.hass)}catch(r){console.error("chronotope: loading stats failed",r)}}get _displayedEvents(){if(!this._filters.dayFilter)return this._events;let r=new Date(`${this._filters.dayFilter}T00:00:00`),a=new Date(r.getTime()+864e5);return this._events.filter(o=>(o.occurrences||[[o.start_time,o.end_time]]).some(([u,c])=>new Date(u)<a&&new Date(c)>r))}render(){let r=!!this.hass?.themes?.darkMode,a=this._displayedEvents;return Z`
      <header>
        <h1>Chronotope</h1>
        <span class="count">
